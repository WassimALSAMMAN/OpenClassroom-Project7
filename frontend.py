...
all_client_numbers = pd.read_csv('./y_test_sx.csv')['SK_ID_CURR']
number_client = st.selectbox('Choose a client number :', all_client_numbers)
filename = "model_xgb_dashb.joblib"
loaded_model = joblib.load(filename)
X_train_sx = pd.read_csv('./X_train_sx.csv')
X_test_sx = pd.read_csv('./X_test_sx.csv')
X_test = X_test_sx.loc[X_test_sx['SK_ID_CURR']==number_client].drop(columns=['SK_ID_CURR'])
...
shap_values = shap_explainer.shap_values(X_train_sx.drop(columns=['SK_ID_CURR']).iloc[0:2], check_additivity=False)
