...
X_train_db = pd.read_csv('./X_train_db.csv').drop(columns=['SK_ID_CURR'])
X_test_db = pd.read_csv('./X_test_db.csv')
all_client_numbers = X_test_db['SK_ID_CURR']
...
y_train_db = pd.read_csv('./y_train_db.csv').drop(columns=['SK_ID_CURR'])
loaded_model = model_xgb.fit(X_train_db, y_train_db)
X_test = X_test_db.loc[X_test_db['SK_ID_CURR']==number_client].drop(columns=['SK_ID_CURR'])
limit_proba = 0.8
...
shap_values = shap_explainer.shap_values(X_train_db)
